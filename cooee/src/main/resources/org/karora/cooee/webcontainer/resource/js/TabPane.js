...
ExtrasTabPane.prototype.addTab = function(tab, tabIndex) {
    ...
    if (this.headerWidth) {
        headerDivElement.style.width = this.headerWidth + "px";
    }
    ...
    if (this.tabInactiveFont) {
        EchoCssUtil.applyStyle(headerDivElement, this.tabInactiveFont);
    }
...
ExtrasTabPane.prototype.calculateHeaderHeight = function(active) {
    if (active) {
	    ...
	    var newHeight = this.headerHeight - this.headerPaddingTop - this.headerPaddingBottom;
	    if(newHeight < 0)
	    	newHeight = 0;
	    return newHeight;
    } else {
	    var largerBorderSize = this.tabInactiveBorderSize > this.tabActiveBorderSize 
                ? this.tabInactiveBorderSize : this.tabActiveBorderSize;
	    var newHeight = this.headerHeight - this.headerPaddingTop - this.headerPaddingBottom - this.activeHeaderHeightIncrease
	            - largerBorderSize;
	    if(newHeight < 0)
	    	newHeight = 0;
	    return newHeight;
...
ExtrasTabPane.prototype.create = function() {
    ...
    if (this.headerHeight == 0)
 		this.headerContainerDivElement.style.visibility = "hidden";    
    switch (this.tabPosition) {
    case ExtrasTabPane.TAB_POSITION_BOTTOM:
...
ExtrasTabPane.prototype.updateTabState = function(tabId, selected) {
	...
	EchoCssUtil.restoreOriginalStyle(headerDivElement);
	...
	if (this.headerWidth) {
	 ...
	 var font = selected ? this.tabActiveFont : this.tabInactiveFont;
    if (font) {
        EchoCssUtil.applyTemporaryStyle(headerDivElement, font);
    }
...
ExtrasTabPane.MessageProcessor.processInit = function(initMessageElement) {
    ...
    if (initMessageElement.getAttribute("header-width")) {
        tabPane.headerWidth = parseInt(initMessageElement.getAttribute("header-width"));
    }
    if (initMessageElement.getAttribute("header-padding")) {
        var headerPadding = new EchoCoreProperties.Insets(initMessageElement.getAttribute("header-padding"));
        tabPane.headerPaddingTop = parseInt(headerPadding.top);
        tabPane.headerPaddingLeft = parseInt(headerPadding.left);
        tabPane.headerPaddingRight = parseInt(headerPadding.right);
        tabPane.headerPaddingBottom = parseInt(headerPadding.bottom);
    }
