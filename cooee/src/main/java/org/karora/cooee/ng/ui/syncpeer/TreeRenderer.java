...
//				cssStyle.setAttribute("height", cellImageRef.getHeight().toString());
	...
	private void renderNodeCell(Object treeNode, TreePath treeNodePath, HtmlTable treeRowTable) {
		...
		if (cellLabel != null) {
            ...
            /*
             * When used with a tree table this EPNG code returns null:
             *    Component cellComponent = tree.getComponent(treeNode);
             * See JIRA issue COO-9 - Fix asks cell renderer for component instead
             */
            Component cellComponent = cellRenderer.getTreeCellRendererComponent(tree, treeNode, isSelected, isExpanded, isLeaf);
