...
implements HttpSessionActivationListener, HttpSessionBindingListener, Serializable {
   ...
   /**
    ...
    void setServletUri(String servletUri) {
    	// Zafin
    	// Andrew McCue
    	//System.out.println( "org.karora.cooee.webrender:UserInstance.setServletUri():Called");
        this.servletUri = xssTrim(servletUri);
    	//System.out.println( "org.karora.cooee.webrender:UserInstance.setServletUri():Set To["+this.servletUri+"]");
    }

    private static String controlCharacters = ";?@&=+-$,[]()'";
    private static String dotHtml = ".html";
    private String xssTrim( String original ){
    	// Zafin
    	// Andrew McCue
    	String result = null;
    	int trimIndex = 0;
    	int minTrimIndex = Integer.MAX_VALUE;
    	int index = 0;
    	//System.out.println( "org.karora.cooee.webrender:UserInstance.xssTrim( "+original+" ):Called");
    	result = original;
    	if( original != null ){
	    	for( index = 0; index < controlCharacters.length(); index++ ){
	    		trimIndex = original.indexOf(controlCharacters.charAt(index));
	    		if( (trimIndex > 0) && (trimIndex < minTrimIndex) ){
	    			minTrimIndex = trimIndex;
	    		}
	    	}
	    	if( minTrimIndex < Integer.MAX_VALUE ){
	    		result = original.substring(0, minTrimIndex);
	    	}else{
	    		result = original;
	    	}
    	}
    	//System.out.println( "org.karora.cooee.webrender:UserInstance.xssTrim("+original+"):returning["+result+"]");
    	return( result );
